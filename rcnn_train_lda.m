...
for i = imdb.class_ids
  ...
  X_pos{i} = get_feature(rcnn_model, imdb.name, im_IX{i}, eigen, lda);
...
d.feat = get_feature(rcnn_model, imdb.name, ...
    struct('image_id', imdb.image_ids{ind}, 'IX', []), eigen, lda);
