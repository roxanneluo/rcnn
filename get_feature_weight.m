function feature = get_feature_weight(pool5, rcnn_model, imdb_name, keys)
    ...
    for j = 1:length(feat_opts)
      ...
      if ~feat_opt.w
        feature(num_start:num_end, dim_start:dim_start+dims(j)-1) = ...
          get_feature_part(feat_opts(j), batch_size-padding, imdb_name, id);
      end
...
if exist_w
  ...
  for i = 1:length(keys)
    ...
    for j = 1:num_feat
      ...
      if feat_opts(j).w
        weight = get_weight_part(imdb_name, img_id, IX, feat_opts(j));
...
% -----------------------------------------------------------------------------
function weight = get_weight_part(imdb_name, image_id, IX, feat_opt)
% -----------------------------------------------------------------------------
...
function feat_part = get_feature_part(feat_opt, valid_num, imdb_name, id)
% -----------------------------------------------------------------------------
