function draw_pos_neg(num_im, num_feat,varargin)
...
my_test_feat_opts = struct('layer', 5, 'd', true, 'w', true, ...
...
for i = imdb.class_ids
  ...
  X_pos{i} = get_feature(X_pos{i}, rcnn_model, imdb.name, im_IX{i});
...
%  for i = 1:length(imdb.image_ids)
  IX = randperm(length(imdb.image_ids), num_im)
  for i = IX
...
feat_name = feat_opts_to_string(rcnn_model.feat_opts);
draw_res = './draw-res/';
sum_folder = [draw_res, feat_name, '-norm', '/summary/'];
all_folder = [draw_res, feat_name, '-norm', '/all/'];
...
err = 1e-6;
for j = imdb.class_ids
  fprintf('class %s-%d, #pos=%d, # pos < %f = %d\n', imdb.classes{j}, j, length(caches{j}.X_pos), err, mean(sum(caches{j}.X_pos < err, 2)));
end
for j = imdb.class_ids
  fprintf('class %s-%d, # pos < %f = %d\n', imdb.classes{j}, j, err, mean(sum(caches{j}.X_pos < err, 2)));
  fprintf('class %s-%d, # neg < %f = %d\n', imdb.classes{j}, j, err, mean(sum(caches{j}.X_neg < err, 2)));
  std(caches{j}.X_pos)
  caches{j}.X_pos(1:10, 1:20)

  figure();
  n = min(size(caches{j}.X_pos,1), size(caches{j}.X_neg,1));
  %n = length(caches{j}.X_pos);
  k = min(n, num_feat);
  IX = randperm(n , k);
  mean_pos = mean(caches{j}.X_pos);
  stdd_pos = std(caches{j}.X_pos);
  draw_summary(j, imdb.classes{j}, mean_pos, stdd_pos, feat_name, sum_folder, 'pos', 'b')
  print_summary(j, imdb.classes{j}, mean_pos, stdd_pos, feat_name, sum_folder, 'pos')

  
  mean_neg = mean(caches{j}.X_neg);
  stdd_neg = std(caches{j}.X_neg);
  draw_summary(j, imdb.classes{j}, mean_neg, stdd_neg, feat_name, sum_folder, 'neg', 'r');
  print_summary(j, imdb.classes{j}, mean_neg, stdd_neg, feat_name, sum_folder, 'neg');
  
  draw_all(j, imdb.classes{j}, double(caches{j}.X_pos(IX,:)), feat_name, all_folder, 'pos');
  draw_all(j, imdb.classes{j}, double(caches{j}.X_neg(IX,:)), feat_name, all_folder, 'neg');
  fprintf('done %d\n', j);
end
% ------------------------------------------------------------------------
...
d.feat = get_feature(d.feat, rcnn_model, imdb.name, ...
    struct('image_id', imdb.image_ids{ind}, 'IX', []));
...
for i = 1:length(imdb.image_ids)
  ...
  d = rcnn_load_cached_pool5_features(opts.cache_name, ...
      imdb.name, load_feat, imdb.image_ids{i}, true);
