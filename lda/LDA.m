function [eigvector, eigvalue] = LDA(gnd,options,data,ReducedDim)
...
%                                                   
MAX_MATRIX_SIZE = 3*3*384; % You can change this number according your machine computational power
...
if bPCA
    fprintf('do PCA (no reg and no full rank), eliminating eigvalue < 1e-3\n');
    ...
    if isfield(options,'bEigs')
        ...
        if (dimMatrix > MAX_MATRIX_SIZE) && (Dim < dimMatrix*EIGVECTOR_RATIO) %TODO ReducedDim?
            bEigs = 1;
        else
            bEigs = 0;
        end
