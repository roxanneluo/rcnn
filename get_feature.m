function feature = get_feature(pool5, rcnn_model, imdb_name, keys, eigen)
...
if exist_r
  ...
  for i=1:num_batches
    ...
    for j = 1:length(feat_opts)
      ...
      %{
        feat_part = get_feature_part(feat_opts(j), batch_size-padding);
        if proj
          assert(~isempty(eigen));
          feat_part = project(feat_part, eigen.eigen_vec, eigen.eigen_val, rcnn_model.whiten);
        end
        feature(num_start:num_end, dim_start:dim_start+dims(j)-1) = feat_part;
        %}
      end
  ...
  for i = 1:length(keys)
    ...
    for j = 1:num_feat
      ...
      %{
        weight = get_weight_part(imdb_name, img_id, IX, feat_opts(j), norm_weight);
        if proj
          weight = project(weight, eigen.eigen_vec, eigen.eigen_val, rcnn_model.whiten);
        end
        num_end = num_start+size(weight,1)-1;
        feature(num_start:num_end, dim_start:dim_end) = weight;
...
file = sprintf('./feat_cache/%s_sb/%s/%s.mat', feat_opts_to_string(feat_opt), ...
    imdb_name, image_id);
