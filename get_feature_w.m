function [feature, boxes] = get_feature_w(pool5, rcnn_model, imdb_name, keys)
...
if exist_w
  ...
  for i = 1:length(keys)
    ...
    for j = 1:num_feat
      ...
      if feat_opts(j).w
        [weight, boxes] = get_weight_part(imdb_name, img_id, IX, feat_opts(j), norm_weight);
...
% -----------------------------------------------------------------------------
function [weight,boxes]  = get_weight_part(imdb_name, image_id, IX, feat_opt, norm_weight)
% -----------------------------------------------------------------------------
